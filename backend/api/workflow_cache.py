...
def store_paused_workflow(request_id: str, workflow: "Workflow") -> None:
    ...
    with _cache_lock:
        ...
        logger.info("Stored paused workflow for request_id=%s (cache size: %d)", 
                   request_id, len(_workflow_cache))
...
def get_paused_workflow(request_id: str) -> "Workflow | None":
    ...
    with _cache_lock:
        ...
        if age > WORKFLOW_TTL:
            logger.warning("Paused workflow for request_id=%s expired (age: %s)", 
                          request_id, age)
            return None
        
        logger.info("Retrieved paused workflow for request_id=%s (age: %s)", 
                   request_id, age)
        return workflow
...
def _cleanup_expired_unlocked() -> None:
    ...
    now = datetime.now()
    expired = [
        rid for rid, (_, stored_at) in _workflow_cache.items()
        if now - stored_at > WORKFLOW_TTL
    ]
    for rid in expired:
