...
def _validate_integer(
    ...
    if not isinstance(value, (int, float)):
        try:
            ...
            violations.append(f"Parameter '{param_name}': expected integer, got '{type(value).__name__}'")
            return violations
    ...
    if validation.min is not None:
        try:
            ...
            if value < min_val:
                violations.append(f"Parameter '{param_name}': value {value} is below minimum {min_val}")
        except (ValueError, TypeError):
            pass
    ...
    if validation.max is not None:
        try:
            ...
            if value > max_val:
                violations.append(f"Parameter '{param_name}': value {value} exceeds maximum {max_val}")
        except (ValueError, TypeError):
            pass
...
def _validate_float(
    ...
    if not isinstance(value, (int, float)):
        try:
            ...
            violations.append(f"Parameter '{param_name}': expected number, got '{type(value).__name__}'")
            return violations
    ...
    async def handle_validation_request(
        ...
        try:
            ...
            if not param_definitions:
                ...
                finish_step()
                response_msg = SQLDraftMessage(source="param_validator", response_json=validated_draft.model_dump_json())
            ...
            finish_step()
            response_msg = SQLDraftMessage(source="param_validator", response_json=error_draft.model_dump_json())
            await ctx.send_message(response_msg)
