            ...
            async for event in workflow.run_stream(nl2sql_request):
                ...
                if isinstance(event, WorkflowOutputEvent):
                    ...
                    try:
                        ...
                        # Track template/params for context updates
                        # (This would need to be passed from the workflow somehow)
                        
                        # Update orchestrator context for potential refinements
                        orchestrator.update_context(response, template_json, extracted_params)
...
async def chat_stream_v2(
    ...
    """
    SSE streaming chat with ConversationOrchestrator (V2 architecture).
