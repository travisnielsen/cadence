...
class SQLDraft(BaseModel):
    ...
    completed_sql: str | None = Field(
        default=None, description="The SQL query with all parameters substituted (if success)"
    )

    user_query: str = Field(default="", description="The original user question")

    reasoning: str | None = Field(
        default=None, description="Explanation of how the query was constructed"
    )

    retry_count: int = Field(
        default=0, description="Number of retry attempts (for validation retry flow)"
    )

    # Template-based fields (source="template")
    template_id: str | None = Field(
        default=None, description="Template ID used (for re-submission on clarification)"
    )
    template_json: str | None = Field(
        default=None, description="Full template JSON (for resumption after clarification)"
    )
    extracted_parameters: dict[str, Any] | None = Field(
        default=None, description="Parameter name -> extracted value mapping"
    )
    ...
    missing_parameters: list[MissingParameter] | None = Field(
        default=None, description="Parameters that need user clarification"
    )
    clarification_prompt: str | None = Field(
        default=None, description="LLM-generated question asking for missing info"
    )

    # Dynamic generation fields (source="dynamic")
    tables_used: list[str] = Field(
        default_factory=list, description="Tables used in the query (for validation)"
    )

    tables_metadata_json: str | None = Field(
        default=None, description="JSON of full TableMetadata objects (for refinement context)"
    )

    # Parameter validation fields
    params_validated: bool = Field(
        default=False, description="Whether parameter values have been validated"
    )
    ...
    parameter_violations: list[str] = Field(
        default_factory=list, description="List of parameter validation failures"
    )

    # Query validation fields
    query_validated: bool = Field(
        default=False, description="Whether the SQL query has been validated"
    )
    query_violations: list[str] = Field(
        default_factory=list, description="List of query validation failures"
    )
    query_warnings: list[str] = Field(
        default_factory=list, description="Non-blocking query validation warnings"
    )
...
class QueryBuilderRequest(BaseModel):
    ...
    tables: list[TableMetadata] = Field(description="Relevant tables to use for query generation")
    retry_count: int = Field(
